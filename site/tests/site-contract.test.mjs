...
test("GitHub Actions builds and deploys the isolated Pages artifact", async () => {
  ...
  assert.match(pagesWorkflow, /actions\\/configure-pages@(?:v5|[a-f0-9]{40}\\s+# v5)/);
  assert.match(pagesWorkflow, /actions\\/upload-pages-artifact@(?:v3|[a-f0-9]{40}\\s+# v3)/);
  assert.match(pagesWorkflow, /path:\s*site\/dist/);
  assert.match(pagesWorkflow, /actions\\/deploy-pages@(?:v4|[a-f0-9]{40}\\s+# v4)/);
