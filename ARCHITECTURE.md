...
**Current state (v0.7.0):** Single-admin authentication is required for dashboard access. Provider system includes manual services, multi-instance Docker (read/actions/logs), and read-only Portainer integration (container status/logs plus stack lifecycle status). Containers and stacks load asynchronously through authenticated server routes with isolated provider failures and short in-memory caches. The stack contract exposes only normalized lifecycle metadata; raw Portainer stack objects, environment variables, deployment content, and credentials never cross the server boundary. See [SECURITY.md](SECURITY.md) for deployment guidance (`SESSION_SECRET`, optional `COOKIE_SECURE` behind HTTPS, Docker socket and Portainer token risks).
