...
**Current state (v0.8.0):** Single-admin authentication is required for dashboard access. Provider system includes manual services, multi-instance Docker (read/actions/logs), and read-only Portainer integration (container status/logs plus endpoint-aware stack lifecycle and read-only stack container membership). Containers and stacks load asynchronously through authenticated server routes with isolated provider failures and short in-memory caches. The Portainer provider exposes the `stack.containers` capability. Its server-only membership lookup first resolves the requested provider/integration and endpoint, then matches a stack name exactly against the Compose `com.docker.compose.project` label or the Swarm `com.docker.stack.namespace` label, and returns only normalized container fields needed by the read-only drawer. Endpoint status has precedence over a stack's lifecycle: a disconnected endpoint makes the effective stack status **Unavailable**, while the last reported lifecycle remains available as context. Membership responses use a short process-local cache but are never persisted in SQLite; cached or previously displayed membership is not shown for a disconnected endpoint. No Portainer credentials, raw Docker/Portainer objects, stack configuration, labels, or environment data cross this boundary. See [SECURITY.md](SECURITY.md) for deployment guidance (`SESSION_SECRET`, optional `COOKIE_SECURE` behind HTTPS, Docker socket and Portainer token risks).

**Next state (Phase 7 remainder):** Carefully scoped Portainer actions with explicit provider opt-in, additional safety checks, and confirmation UX.
