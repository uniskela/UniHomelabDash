...
**Current state (v0.10.0):** Single-admin authentication is required for dashboard access. Provider system includes manual services, multi-instance Docker (read/actions/logs/inspect/stats), and Portainer (container status/logs/inspect/stats, opt-in container start/stop/restart, plus endpoint-aware stack lifecycle and read-only stack container membership). Containers and stacks load asynchronously through authenticated server routes with isolated provider failures and short in-memory caches. Container detail routes are `providerId`-scoped: the server loads the provider row from SQLite and derives type from that row rather than trusting a client-supplied provider type for inspect, stats, and actions. The Docker and Portainer providers expose `container.inspect` and `container.stats`. Inspect responses are sanitized (label-value allowlist for Compose project, Swarm namespace, and OCI title/version/vendor; no env, cmd, healthcheck output, or bind mount Source paths). Inspect results cache for 30 seconds and stats for 5 seconds in process-local memory. The Portainer provider also exposes the `stack.containers` capability. Its server-only membership lookup first resolves the requested provider/integration and endpoint, then matches a stack name exactly against the Compose `com.docker.compose.project` label or the Swarm `com.docker.stack.namespace` label, and returns only normalized container fields needed by the read-only drawer. Endpoint status has precedence over a stack's lifecycle: a disconnected endpoint makes the effective stack status **Unavailable**, while the last reported lifecycle remains available as context. Membership responses use a short process-local cache but are never persisted in SQLite; cached or previously displayed membership is not shown for a disconnected endpoint. Alerts foundation (v0.10.0) persists `activity_events` and `alerts` in SQLite, records health transitions on manual checks, provider connection test outcomes, and container actions, and exposes an in-app feed at `/alerts` with acknowledge/resolve controls. External notifications remain deferred. See [SECURITY.md](SECURITY.md) for deployment guidance (`SESSION_SECRET`, optional `COOKIE_SECURE` behind HTTPS, Docker socket and Portainer token risks).

**Next state (Phase 7 remainder / Phase 12):** Portainer stack restart/redeploy remain deferred pending action-safety design. Background health polling and external notifications are the next increments on the alerts foundation.

Required rules:

* Require authentication before all dashboard access (shipped in v0.2.0).
