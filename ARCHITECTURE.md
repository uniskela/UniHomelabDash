...
**Current state (v0.6.2):** Single-admin authentication is required for dashboard access. Provider system includes manual services, multi-instance Docker (read/actions/logs), and read-only Portainer integration (container status/logs). Containers load asynchronously with short list timeouts, endpoint cooldown, and in-memory caching. Containers UI supports view modes, grouping, hide preferences (stored in the settings table), and prefixed search. See [SECURITY.md](SECURITY.md) for deployment guidance (`SESSION_SECRET`, optional `COOKIE_SECURE` behind HTTPS, Docker socket and Portainer token risks).
